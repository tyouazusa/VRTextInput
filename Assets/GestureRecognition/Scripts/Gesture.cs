...
public class Gesture {
    ...
    public Result Recognize(GestureLibrary gestureLibrary, bool fast = false) {
        ...
        if (this.Points.Count <= 2) {
            ...
            for (int i = 0; i < library.Count; i++) {
                ...
                if (fast) {
                    d = GetOptimalCosineDistance(library[i].Vector, this.Vector);//采用这种算法
                } else {
            ...
            if (u == -1) {
                ...
                return new Result(library[u].Name, fast ? 1f / b : 1f - b / this.HALF_DIAGONAL);//分值为1/b
            }
        }
    }
    ...
    public List<Vector2> Resample(int numberOfPoints) {
        ...
        float I = Gesture.GetPathLength(this.Points) / (numberOfPoints - 1);//平均每个点之间的长度
        float D = 0.0f;
        ...
        for (int i = 1; i < this.Points.Count; i++) {
            float d = Vector2.Distance(this.Points[i - 1], this.Points[i]);//两个相邻点之间的距离
    ...
    public List<Vector2> RotateBy(float angle) {
        ...
        for (int i = 0; i < this.Points.Count; i++) {
            ...
            rotatedPoints.Add(new Vector2(x, y));//旋转后的点 还是64个
        }
    ...
    public List<float> Vectorize() {
        ...
        float magnitude = Mathf.Sqrt(sum);//m=根号下(x2+y2)
